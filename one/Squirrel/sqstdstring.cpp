...
{
    const SQChar *str = nullptr,*start = nullptr,*end = nullptr;
...
{
    const SQChar *str = nullptr,*start = nullptr;
    sq_getstring(v,2,&str);
    __strip_l(str,&start);
...
{
    const SQChar *str = nullptr,*end = nullptr;
    sq_getstring(v,2,&str);
    SQInteger len = sq_getsize(v,2);
    __strip_r(str,len,&end);
...
{
    const SQChar *str = nullptr,*seps = nullptr;
    SQChar *stemp;
    sq_getstring(v,2,&str);
...
static SQInteger _string_escape(HSQUIRRELVM v)
{
    ...
    SQChar *dest = nullptr,*resstr = nullptr;
    SQInteger size;
    sq_getstring(v,2,&str);
    size = sq_getsize(v,2);
    if(size == 0) {
...
{
    const SQChar *str = nullptr,*cmp = nullptr;
    sq_getstring(v,2,&str);
...
{
    SETUP_REX(v);
    const SQChar *str = nullptr,*begin = nullptr,*end = nullptr;
    SQInteger start = 0;
    sq_getstring(v,2,&str);
...
{
    const SQChar *error = nullptr,*pattern = nullptr;
